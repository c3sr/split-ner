...
\bottomrule
\end{tabular}
\caption{Results. \texttt{\#F1} mention-level Micro F1\%. \yjcomment{we need to add discussions for MRC-Dice and CL-KL as they use additional knowledge. point out that our system does not rely on external knowledge which is usually unavailable for domain-speicific data and results in longer training}}
\label{tab:main}
\end{small}
\end{table*}
...
Span-Detector-Vanilla     & 90.67 & todo & todo & todo  \\
\bottomrule
\end{tabular}
\caption{Span Detector Results. \texttt{\#F1} mention-level Micro F1\%. \yjcomment{what settings are for Vanilla?}}
...
\begin{tabular}{ccccc}\toprule
 \textbf{Model} & \texttt{BioNLP13CG} & \texttt{CyberThreats} & \texttt{OntoNotes} & \texttt{WNUT17} \\ \toprule 
Span-Class-Dice & 94.27 & todo &  96.74 & 73.40  \\
Span-Class-CE     & 94.04 & 87.58 & todo & todo  \\
\bottomrule
\end{tabular}
