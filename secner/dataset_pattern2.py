...
logging.basicConfig(filename='dataset.log',  level=logging.DEBUG)
...
class NerDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #logger.debug("input_ids="+str(bert_token_ids))
        #logger.debug("token_type_ids="+str(bert_token_type_ids))
        #logger.debug("head_mask="+str(bert_head_mask))
        #logger.debug("text="+str(bert_token_text))
        #logger.debug("sub_text="+str(bert_sub_token_text))
        #logger.debug("pos_tag="+str(bert_token_pos))
        #logger.debug("dep_tag="+str(bert_token_dep))
        #logger.debug("labels="+str(bert_tag_ids))
...
class NerDataCollator:
    ...
    def __call__(self, features):
        ...
        # YJ changes: 
        # (1) make sure that the padding value should not be in the vocabulary (default: 0)
        # (2) move to pre-padding as it is preferred; 
        # (3) better to group input data by lengths so that all sequences in a batch are in the (almost) same length and then shuffle the groups
