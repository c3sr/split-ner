...
class ProcessedSentenceAndTagSpan:

    def __init__(self, text_sentence, char_indexed_sentence, pattern_indexed_sentence, word_indexed_sentence,
                 type_indexed_sentence, word_level_sentence_mask, char_level_sentence_mask, begin_position,
                 end_position, span_position):
...
class SpanTypeDataset(Dataset):
    ...
    def read_tag_emb(self, tags, emb_dict, tag_emb_dim):
        tag_emb = []
        for tag in tags:
            main_vec = emb_dict[tag] if tag in emb_dict else [0.0] * tag_emb_dim

            if tag == self.pad_tag:
                tag_emb.append(main_vec + [1.0])
            else:
                tag_emb.append(main_vec + [0.0])
        return tag_emb
    ...
    def add_processed_sentence_tag(self, out):
        ...
        self.begin_position.append(out.begin_position)
    ...
    def __getitem__(self, index):
        ...
        begin_position = self.begin_position[index]
        end_position = self.end_position[index]
        span_position = self.span_position[index]
    ...
    def process_sentence_and_tag(self, token_sentence, text_tag):
        ...
        begin_position = np.zeros(self.max_seq_len, dtype=np.long)
        end_position = np.zeros(self.max_seq_len, dtype=np.long)
        span_position = np.zeros((self.max_seq_len, self.max_seq_len, len(self.out_tags)), dtype=np.long)
        begin_index = -1
        for index, tag in enumerate(text_tag):
            if tag.startswith("B-"):
                begin_index = index
                begin_position[index] = 1
                end_position[index] = 1
                tag_index = self.out_tags.index(tag[2:])
                span_position[begin_index, index, tag_index] = 1
            elif tag.startswith("I-"):
                end_position[index - 1] = 0
                end_position[index] = 1
                tag_index = self.out_tags.index(tag[2:])
                span_position[begin_index, index - 1, tag_index] = 0
                span_position[begin_index, index, tag_index] = 1
            else:
                begin_index = -1

        return ProcessedSentenceAndTagSpan(text_sentence, char_indexed_sentence, pattern_indexed_sentence,
                                           word_indexed_sentence, type_indexed_sentence, word_level_sentence_mask,
                                           char_level_sentence_mask, begin_position, end_position, span_position)
