...
class CNNBiLSTMCRF(nn.Module):
    ...
    def __init__(self, tag_to_index, inp_dim, conv1_dim, hidden_dim, out_dim, kernel_size, word_len, device,
                 word_vocab_size=None, word_emb_dim=None, pre_trained_emb=None, use_word=True, use_char=True,
                 use_maxpool=False, use_lstm=False):
        super(CNNBiLSTMCRF, self).__init__()

        self.cnn_lstm = PatternCNN(inp_dim=inp_dim, conv1_dim=conv1_dim, out_dim=out_dim, hidden_dim=hidden_dim,
                                   kernel_size=kernel_size, word_len=word_len, word_vocab_size=word_vocab_size,
                                   use_lstm=use_lstm, word_emb_dim=word_emb_dim, pre_trained_emb=pre_trained_emb,
                                   use_char=use_char, use_word=use_word, use_maxpool=use_maxpool, device=device)

        self.crf = CRF(tag_to_index=tag_to_index, device=device)

    def neg_log_likelihood(self, word_x, char_x, word_mask, char_mask, tags):
        feats = self.cnn_lstm(word_x, char_x, word_mask, char_mask)
...
class PatternCNNExecutor(BaseExecutor):
    ...
    def __init__(self, args):
        ...
        tag_to_index = {k: i for i, k in enumerate(self.train_dataset.out_tags)}

        self.model = CNNBiLSTMCRF(tag_to_index=tag_to_index, inp_dim=self.train_dataset.inp_dim,
                                  conv1_dim=self.args.conv1_dim, out_dim=len(self.train_dataset.out_tags),
    ...
    def train_epoch(self, epoch):
        ...
        with tqdm(self.train_data_loader) as progress_bar:
            for text, word_feature, char_feature, word_mask, char_mask, label in progress_bar:
                ...
                loss = self.model.neg_log_likelihood(word_feature, char_feature, word_mask, char_mask, label)
    ...
    def evaluate_epoch(self, data_loader, epoch, prefix, outfile=None):
        ...
        for text, word_feature, char_feature, word_mask, char_mask, label in data_loader:
            ...
            label = label.to(self.device)
            with torch.no_grad():
                score, prediction = self.model(word_feature, char_feature, word_mask, char_mask)
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Pattern-Context-CNN-LSTM-CRF Model for Sequence Labeling")
    ap.add_argument("--name", type=str, default="pattern-cnn-lstm-crf",
                    help="model name (Default: 'pattern-cnn-lstm-crf')")
