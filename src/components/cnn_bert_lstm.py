...
class CNN_LSTM_Base(nn.Module):
    ...
    def forward(self, text, word_x, char_x, type_x, word_mask, char_mask):
        ...
        if self.use_word != "none":
            if "bert" in self.use_word:
                word_emb_x = CNN_LSTM_Base.get_bert_embeddings(word_x[:, :, 0], self.bert_model,
    ...
    @staticmethod
    def get_bert_embeddings(batch_ids, bert_model, device, use_tfo="none", tfo_model=None):
        bert_batch_vectors = bert_model(batch_ids.to(device))[0]

        if use_tfo == "xl":
            bert_batch_vectors = tfo_model(inputs_embeds=bert_batch_vectors)[0]

        return bert_batch_vectors
